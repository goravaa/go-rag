...
// was not loaded in eager-loading.
func (e ProjectEdges) QueriesOrErr() ([]*UserPrompt, error) {
	if e.loadedTypes[2] {
	...
	for i := range columns {
		...
		case project.ForeignKeys[0]:
			if value, ok := values[i].(*sql.NullScanner); !ok {
				...
				_m.user_projects = new(uuid.UUID)
				*_m.user_projects = *value.S.(*uuid.UUID)
			}
		default:
...
func (_m *Project) QueryQueries() *UserPromptQuery {
	return NewProjectClient(_m.config).QueryQueries(_m)
}
