...
func (_q *UserQuery) QueryQueries() *UserPromptQuery {
...
func (_q *UserQuery) WithQueries(opts ...func(*UserPromptQuery)) *UserQuery {
	query := (&UserPromptClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (_q *UserQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*User, error) {
	...
	if query := _q.withQueries; query != nil {
		if err := _q.loadQueries(ctx, query, nodes,
			func(n *User) { n.Edges.Queries = []*UserPrompt{} },
			func(n *User, e *UserPrompt) { n.Edges.Queries = append(n.Edges.Queries, e) }); err != nil {
			return nil, err
		}
	}
...
func (_q *UserQuery) loadQueries(ctx context.Context, query *UserPromptQuery, nodes []*User, init func(*User), assign func(*User, *UserPrompt)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[uuid.UUID]*User)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
...
func (_q *UserQuery) loadSessions(ctx context.Context, query *SessionQuery, nodes []*User, init func(*User), assign func(*User, *Session)) error {
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(session.FieldSessionsUserids)
	}
