...
func (_q *ProjectQuery) QueryQueries() *UserPromptQuery {
...
func (_q *ProjectQuery) WithQueries(opts ...func(*UserPromptQuery)) *ProjectQuery {
	query := (&UserPromptClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (_q *ProjectQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Project, error) {
	...
	if query := _q.withQueries; query != nil {
		if err := _q.loadQueries(ctx, query, nodes,
			func(n *Project) { n.Edges.Queries = []*UserPrompt{} },
			func(n *Project, e *UserPrompt) { n.Edges.Queries = append(n.Edges.Queries, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (_q *ProjectQuery) loadDocuments(ctx context.Context, query *DocumentQuery, nodes []*Project, init func(*Project), assign func(*Project, *Document)) error {
	...
	for _, n := range neighbors {
		fk := n.project_documents
		if fk == nil {
			return fmt.Errorf(`foreign-key "project_documents" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "project_documents" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
}
func (_q *ProjectQuery) loadQueries(ctx context.Context, query *UserPromptQuery, nodes []*Project, init func(*Project), assign func(*Project, *UserPrompt)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[int]*Project)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
	...
	query.withFKs = true
	query.Where(predicate.UserPrompt(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(project.QueriesColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
	...
	for _, n := range neighbors {
		...
		if fk == nil {
			return fmt.Errorf(`foreign-key "project_queries" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "project_queries" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
